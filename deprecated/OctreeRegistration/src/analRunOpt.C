...
#define TAU_IMG(x, y, z) (255.0*(SQR((sin(4.0*__PI__*(x)))*(sin(4.0*__PI__*(y)))*(sin(4.0*__PI__*(z))))))

#define SIG_IMG(x, y, z) TAU_IMG(((x) + (0.05*(sin(2.0*__PI__*(x)))*(sin(2.0*__PI__*(y)))*(sin(2.0*__PI__*(z))))), (y), (z))
...
int main(int argc, char** argv) {
  ...
  if(argc < 2) {
    if(!rank) {
      std::cout<<"Usage: exe Nfe"<<std::endl;
    }
    PetscFinalize();
    exit(0);
  }
  ...
  for(int lev = (numOptLevels - 1); lev >= 0; lev--) {
    ...
    if(Ne == Nfe) {
      ...
      for(int zi = fzs; zi < (fzs + fnze); zi++) {
        for(int yi = fys; yi < (fys + fnye); yi++) {
          for(int xi = fxs; xi < (fxs + fnxe); xi++) {
            ...
            double sigVal = SIG_IMG(xPt, yPt, zPt);
            double tauVal = TAU_IMG(xPt, yPt, zPt);
      ...
      if(rank < npesCurr[lev + 1]) {
        coarsenPrlImage(da1dof[lev + 1], da1dof[lev], (rank < npesCurr[lev]), sigImgPrev, sigImgCurr);
        coarsenPrlImage(da1dof[lev + 1], da1dof[lev], (rank < npesCurr[lev]), tauImgPrev, tauImgCurr);
      }
    }
