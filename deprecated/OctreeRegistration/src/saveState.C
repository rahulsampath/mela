  ...
  for(int lev = 0; lev < numOptLevels; lev++) {
    ...
    if(rank < npesCurr[lev]) {
      ...
      if(lev == 4) {
        ...
        writeNodesToFile(debugFname, balOct);
      }

      if(!rank) {
        std::cout<<"Finished saving oct."<<std::endl;
      }
      ...
      if(!rank) {
        std::cout<<"Copying vals to Patches..."<<std::endl;
      }
      ...
      if(!rank) {
        std::cout<<"Creating new hess..."<<std::endl;
      }
    ...
    if(!rank) {
      std::cout<<"Creating nodePts..."<<std::endl;
    }
    ...
    if(!rank) {
      std::cout<<"Creating nodeVals..."<<std::endl;
    }
    ...
    if(!rank) {
      std::cout<<"Resetting damgprev..."<<std::endl;
    }
    ...
    if(damg) {
      ...
      if(lev) {
        ...
        if(dao->iAmActive()) {
          ...
          for(dao->init<ot::DA_FLAGS::WRITABLE>();
            ...
            if(!(hnMask & 1)) {
              for(int d = 0; d < 3; d++) {
                assert( ((3*ptsCtr) + d) < nodeVals.size() );
      ...
      if(lev == 4) {
        if(dao->iAmActive()) {
          ...
          saveVector(Uin, uinFname);
        }
      }

      if(!rank) {
        std::cout<<"Starting newGaussNewton..."<<std::endl;
      }
...
*/
  ...
  if(!rank) {
    std::cout<<"Cleaning up DAMG "<<std::endl;
  }
